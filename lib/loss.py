...
def loss_calculation(pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, num_point_mesh, sym_list):
    ...
    # print()
    # print("type : " + str(type(model_points)) + " / " + str(type(base)))
    # print("input : " + str(model_points.shape))
    # print("mat2 : " + str(base.shape))

    # s1,s2,s3 = model_points.size()
    # bmm = torch.empty(s1,s2,s3, dtype=torch.float)

    bmm = torch.bmm(model_points.cpu(), base.cpu()).cuda()
    pred = torch.add(bmm, points + pred_t)
